...
import { StatusBicicletaEnum } from "@/lib/statusBicicleta";
...
export class PutBicicletasCommand implements BikampCommand {
  constructor(
    private bicicletasService: BicicletaService, private id: number,
    private status: StatusBicicletaEnum ,
    private bicicleta_patrimonio: string
  ) { }

  async execute(): Promise<void> {
    return await this.bicicletasService.putBicicleta({
      id: this.id,
      status: this.status,
